...
function context.OnScriptUnload()
    print("LunyScript OnScriptUnload")
end

-- This runs AFTER the script was (re-)loaded, before any other events (ie Awake). Perhaps you need to restore
-- some state before the script's Awake runs. Commonly used in combintation with OnScriptUnload.
function context.OnScriptLoad()
    print("LunyScript OnScriptLoad")
end
